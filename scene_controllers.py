...
def handleRotation(gamedata, event):
    ...
    for ax in range(3):
        if event.isKeypress(posKeys[k]):
            drot[ax] += rspeed
        elif event.isKeypress(negKeys[k]):
            drot[ax] -= rspeed
        else:
            isRelease = False
            isRelease |= event.isKeyrelease(posKeys[k])
            isRelease |= event.isKeyrelease(negKeys[k])
            if isRelease:
                drot[ax] = 0
...
'''
BELOW IS NOTHING BUT TERROR
FLEE TRAVELER
ONLY DARKNESS AND TRAGEDY LIES BEYOND
'''
